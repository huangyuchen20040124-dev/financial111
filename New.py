...
for i in range(1, len(KBar_dic['time']) - 1):

    if np.isnan(KBar_dic['MACD'][i]):
        continue
    ...
    if OrderRecord.GetOpenInterest() == 0:
        ...
        # 黃金交叉做多
        if KBar_dic['MACD'][i-1] < KBar_dic['Signal'][i-1] and            KBar_dic['MACD'][i] > KBar_dic['Signal'][i]:

            OrderRecord.Order(
                'Buy',
                KBar_dic['product'][i+1],
                KBar_dic['time'][i+1],
                KBar_dic['open'][i+1],
                1
            )
        ...
        # 死亡交叉放空
        elif KBar_dic['MACD'][i-1] > KBar_dic['Signal'][i-1] and              KBar_dic['MACD'][i] < KBar_dic['Signal'][i]:
    ...
    # 多單出場
        ...
        if KBar_dic['MACD'][i] < KBar_dic['Signal'][i]:
    ...
    # 空單出場
        ...
        if KBar_dic['MACD'][i] > KBar_dic['Signal'][i]:
            ...
            OrderRecord.Cover(
                ...
                KBar_dic['time'][i+1],
                KBar_dic['open'][i+1],
                abs(OrderRecord.GetOpenInterest())
            )

print(OrderRecord.GetTradeRecord())
print("總損益:", OrderRecord.GetTotalProfit())
...
for i in range(1, len(KBar_dic['time']) - 1):
    ...
    if OrderRecord.GetOpenInterest() == 0:
        ...
        # 黃金交叉做多
        if KBar_dic['K'][i-1] < KBar_dic['D'][i-1] and            KBar_dic['K'][i] > KBar_dic['D'][i]:
        ...
        # 死亡交叉放空
        elif KBar_dic['K'][i-1] > KBar_dic['D'][i-1] and              KBar_dic['K'][i] < KBar_dic['D'][i]:
